...
def get_my_appointments_in_booking_of_Dr_web_app():
    try:
        req_data = request.get_json()
        arg_list = None
        if not req_data:
        ...
        appointment_list = PatientAppointmentModel.load_my_appointments_in_booking_of_Dr_web_app(_provider_id, _clinic_id, _date_pref)
...
def get_cancel_appointment_in_booking_by_patient_no():
    try:
        req_data = request.get_json()
        arg_list = None
        if not req_data:
        ...
        if not cancel_appointment_list:
            message = {'error': 'No appointments found for the person number'}
            return common.info_request_response(message, 404)
...
@patient_appointment_api.route('/web_app/booking/book_new_appointment/', methods=['GET'])
def get_provider_list_in_book_new_appointment():
    try:
        ...
        provider_List_in_book_new_appointment = PatientAppointmentModel.load_provider_list_in_book_new_appointmnet(_clinic_id, _date_pref)
